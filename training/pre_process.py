...
class land_transform(object):
    ...
    # def __call__(self, land):
        ...
        # land[0:len(land):2] = land[0:len(land):2] * self.scale_factor
        # land[1:len(land):2] = land[1:len(land):2] * self.scale_factor
        land[:len(land)] = land[:len(land)].astype('float32') * self.scale_factor
        return land

class image_train(object):
    def __init__(self, crop_size):
        self.crop_size = crop_size
    def __call__(self, img, flip, offset_x, offset_y, aug):
    # def __call__(self, img):
        ...
        if (aug):
            ...
            transform = transforms.Compose([
                ...
                SetFlip(flip),
                transforms.ColorJitter(brightness = brit),
